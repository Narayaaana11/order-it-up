...
const FLO_PATTERNS = [
  /(?:^|[\s\\/])order[\s_\-]*it[\s_\-]*up(?:\.exe)?(?:$|\s)/i,
  /(?:^|[\s\\/])order-it-up(?:\.exe)?(?:$|\s)/i,
  /(?:^|[\s\\/])flocafe(?:\.exe)?(?:$|\s)/i,
  ...
  /(?:^|\s)(?:node|nodejs)(?:\s+\S+)*[\\/](?:FloCafe|OrderItUp)[\\/](?:dev-server\.js|dist[\\/](?:main[\\/])?index\.js)(?:$|\s)/i,
...
async function killPort(port) {
  ...
  if (floProcs.length === 0) {
    ...
    console.log(
      `[kill-ports] Note: Non-Flo process is holding port ${port}. Order It Up will attempt fallback ports automatically on startup, or you can configure custom ports (PORT, KDS_PORT, SERVER_APP_PORT) in .env.`
    );
    return;
  }
