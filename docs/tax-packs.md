...
2. Give it a unique `id` using the lowercase, hyphenated full country name (for example, `official-india`, `official-thailand`, or `official-united-states`). Keep `country` as its ISO alpha-2 code (for example, `IN`, `TH`, or `US`), because FloCafe uses that field to match the store. Set `publisher` to your name/org (anything other than `local` — `local` is reserved for the in-app manual/unbundled pack and can never be published), and fill in `jurisdiction`, `currency`, `taxRounding`, `payableRounding`.
   ...
   If the pack's rates were compiled from publicly available information rather than a reviewed/authoritative source, set `sourceType: "community"` and use the `community-<country>` id prefix instead (for example, `community-nepal`) — never `official-<country>`, which is reserved for a professionally reviewed pack for that country. A community pack cannot be activated until the owner acknowledges an in-app no-liability disclaimer once per pack id (see `activateInstalledPack()` in `main/routes/tax-packs.ts`); leaving `sourceType` unset (or `"official"`) is the default and skips that gate. `validationChecklist()` rejects a `publisher: "local"` pack that sets `sourceType: "community"` — the disclaimer only applies to catalog-distributed packs, not a merchant's own manual configuration.
