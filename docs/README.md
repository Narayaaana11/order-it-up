# Order It Up documentation index

This index classifies documentation in `docs/` so developers and AI agents know which documents describe current runtime behavior, which describe planned designs, and which are historical records.
...
> **Source of truth principle**: Current code and automated tests define what Order It Up does now. Approved task descriptions and issues define what a specific change is intended to accomplish. Cross-project invariants in `AGENTS.md` and `CURRENT` documentation govern broader constraints.
