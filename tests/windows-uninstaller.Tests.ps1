...
Describe 'Order It Up Windows uninstaller' {
  It 'resolves Keep/Delete before looking for a process to terminate' {
    ...
    $result = Invoke-OrderItUpUninstall
  ...
  It 'bounds a hung child uninstaller and continues with manual cleanup' {
    $uninstallerExe = 'C:\Order It Up\uninstall.exe'
    $fallbackInstallPath = 'C:\Order It Up Fixture\Programs\Order It Up'
    $intermediateId = 9899
    $descendantId = 9900
    $lateDescendantId = 9902
    $entry = [pscustomobject]@{
      DisplayName     = 'Order It Up'
      PSChildName     = 'OrderItUp'
      PSPath          = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\OrderItUp'
      InstallLocation = 'C:\Order It Up'
      UninstallString = '"C:\Order It Up\uninstall.exe" /D=C:\Order It Up'
    }
    ...
    try {
      $env:LOCALAPPDATA = 'C:\Order It Up Fixture'
  ...
  It 'blocks cleanup when child process inspection times out' {
    ...
    $entry = [pscustomobject]@{
      ...
      PSPath          = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\OrderItUp'
      InstallLocation = 'C:\Order It Up'
      UninstallString = '"C:\Order It Up\uninstall.exe"'
    }
  ...
  It 'returns an incomplete result when a locked path remains after bounded retries' {
    ...
    try {
      ...
      $result = Invoke-Removal 'C:\Order It Up' 'install directory'
  ...
  It 'skips purge when Order It Up cannot be confirmed stopped' {
    $entry = [pscustomobject]@{
      DisplayName     = 'Order It Up'
      PSChildName     = 'OrderItUp'
      PSPath          = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\OrderItUp'
    ...
    $result = Invoke-OrderItUpUninstall -PurgeData
  ...
  It 'keeps readable registry results when another uninstall root is inaccessible' {
    $readableEntry = [pscustomobject]@{
      DisplayName     = 'Order It Up'
      PSChildName     = 'OrderItUp'
      PSPath          = 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\OrderItUp'
      InstallLocation = ''
      UninstallString = ''
    }
  ...
  It 'processes and verifies every matching registry installation entry' {
    $firstInstallPath = 'C:\Order It Up First'
    $secondInstallPath = 'C:\Order It Up Second'
    $firstUninstaller = 'C:\Order It Up First\uninstall.exe'
    $secondUninstaller = 'C:\Order It Up Second\uninstall.exe'
    $testEntries = @(
      [pscustomobject]@{
        DisplayName     = 'Order It Up'
        PSChildName     = 'OrderItUpFirst'
        PSPath          = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\OrderItUpFirst'
        InstallLocation = $firstInstallPath
        UninstallString = "`"$firstUninstaller`""
      }
      [pscustomobject]@{
        DisplayName     = 'Order It Up'
        PSChildName     = 'OrderItUpSecond'
        PSPath          = 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\OrderItUpSecond'
  ...
  It 'does not launch an uninstaller outside a known install location' {
    ...
    $entry = [pscustomobject]@{
      ...
      PSChildName     = 'OrderItUp'
      PSPath          = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\OrderItUp'
      InstallLocation = 'C:\Program Files\Order It Up'
