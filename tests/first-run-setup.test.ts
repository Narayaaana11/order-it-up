...
assert.equal(getCurrentSchemaVersion(), MIGRATIONS[MIGRATIONS.length - 1].version, 'fresh database migrates to latest schema');
  ...
  assert.equal(setting('cloud_server_url'), 'https://api.orderitup.in', 'cloud server URL is seeded');
  ...
  try {
    ...
    assert.equal(setting('cloud_server_url'), 'https://api.orderitup.in', 'cloud server URL keeps the default');
