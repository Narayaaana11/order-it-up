...
function createDatabaseMissingError(dbPath: string): Error & { code: string } {
  const error = new Error(
    ...
    'Restore from a backup (Settings → Backup & Restore, or the backups folder) and restart Order It Up.'
...
export class SchemaVersionMismatchError extends Error {
  constructor(public readonly dbVersion: number, public readonly appVersion: number) {
    super(
      ...
      `Please update Order It Up to the latest version before continuing.`
    );
