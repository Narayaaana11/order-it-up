...
export const DRIVE_BACKUP_FOLDER_NAME = 'Order It Up Backups';
...
class GoogleDriveService {
  ...
  private async connectInternal(signal: AbortSignal): Promise<GoogleDriveStatus> {
    ...
    if (!tokens.refresh_token) {
      ...
      throw new Error('Google did not return a refresh token. Revoke Order It Up access at myaccount.google.com/permissions and try connecting again.');
    }
      ...
      const server = http.createServer((req, res) => {
        ...
        res.end(
          error || !code || returnedState !== state
            ? '<html><body>Google Drive connection failed. You can close this window and try again in Order It Up.</body></html>'
            : '<html><body>Google Drive connected. You can close this window and return to Order It Up.</body></html>'
        );
