...
async function showRuntimeStuckDialog(reason: string): Promise<void> {
  ...
  detailLines.push(`Platform: ${process.platform} | Order It Up: ${app.getVersion()}`);

  const { response } = await dialog.showMessageBox({
    type: 'error',
    title: 'Order It Up needs to restart',
    message: 'Order It Up could not recover automatically. This usually means another program '
      + '(antivirus, firewall, or a leftover process) is blocking its local server.\n\n'
...
if (process.env.FLO_E2E_USER_DATA_DIR) {
  ...
  app.name = 'order-it-up';
  app.setPath('userData', path.join(os.homedir(), '.config', 'order-it-up'));
}
...
function createTray(): void {
  ...
  try {
    ...
    const contextMenu = Menu.buildFromTemplate([
      { label: 'Open Order It Up', click: () => { showMainWindow(); } },
      { type: 'separator' },
...
function createMenu(): void {
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    },
    {
      label: 'Window',
      submenu: [
        { label: 'Order It Up', click: () => { if (showMainWindow()) mainWindow?.focus(); } },
        { type: 'separator' },
        { role: 'minimize' },
        ...(process.platform === 'darwin' ? [
          { role: 'zoom' as const },
    ...
    },
    {
      label: 'Help',
      submenu: [
        ...(process.platform !== 'darwin' ? [{ label: 'About Order It Up', click: () => showAbout() }] : []),
        ...(isStoreBuild
          ? []
...
function showAbout(): void {
  ...
  dialog.showMessageBox({
    ...
    detail: [
      ...
      '',
      'A modern, offline-first restaurant Point of Sale & billing system.',
      'Your data stays securely on your local device.',
      '',
      `POS URL: http://${ip}:${getServerPort()}`,
      `KDS URL: http://${ip}:${kdsPort}`,
      `Server App URL: http://${ip}:${serverAppPort}`,
    ].join('\n'),
    buttons: ['OK'],
  });
}
...
const cleanupCoordinator = createShutdownCoordinator(() => [
  ...
  { name: 'MongoDB sync', run: () => mongoSyncService.stop(), blocksDatabase: true },
  { name: 'Amazon S3', run: () => amazonS3Service.stop(), blocksDatabase: true },
